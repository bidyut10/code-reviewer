...
exports.getRepos = async (req, res) => {
  ...
  try {
    ...
    const events = await fetchGitHubUserProfile(
...
exports.getCommits = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Failed to fetch commits for repository: ${repo}` });
  }
};
...
const generateResponse = async (msg) => {
  try {
    ...
    const model = genAI.getGenerativeModel({
      model: process.env.REACT_API_MODEL,
    });
...
exports.getCommitFiles = async (req, res) => {
  ...
  try {
    const changedFiles = await fetchCommitFiles(
      req.user.accessToken,
      req.user.username,
      repo,
      sha
    ...
    res
      .status(500)
      .json({
        error: `Failed to fetch commit details or generate code review for commit ${sha} in repository ${repo}`,
      });
  }
};
